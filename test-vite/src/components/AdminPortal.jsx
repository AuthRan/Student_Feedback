...
const AdminPortal = () => {
  ...
  //Fetching real feedback data from database
  const [activeFeedbackCampaigns, setActiveFeedbackCampaigns] = useState([]);
  const [campaignsLoading, setCampaignsLoading] = useState(true);
  const [campaignsError, setCampaignsError] = useState("");
  // Add this useEffect block to fetch campaigns
  useEffect(() => {
    const fetchCampaigns = async () => {
      // Only fetch if the feedback tab is active
      ...
      if (!token) {
        setCampaignsError("Authentication failed. Please log in again.");
      ...
      try {
        ...
        setActiveFeedbackCampaigns(data);
      } catch (err) {
    ...
    fetchCampaigns();
  }, [activeTab]); // This hook runs every time the activeTab changes
  ...
  const generateSurveyLink = () => {
    ...
    const linkId = `${department
      .toLowerCase()
      .replace(/\s+/g, "-")}-${year}-${section.toLowerCase()}-${subject
      .toLowerCase()
      .replace(/\s+/g, "-")}-${Date.now()}`;
    return `${window.location.origin}/survey/${linkId}`;
  };

  // Replace your old handleCreateFeedback with this one
  const handleCreateFeedback = async () => {
    ...
    if (feedbackForm.questions.some((q) => !q.question.trim())) {
      setFormError("Please ensure all question fields are filled out.");
      setTimeout(() => setFormError(""), 5000);
      return;
    }
    ...
    const campaignData = {
      ...
      targetDepartment: feedbackForm.department,
      targetYear: feedbackForm.year, // The backend expects 'targetYear'
      questions: feedbackForm.questions.map((q) => q.question), // Send only the question text
      deadline: feedbackForm.deadline,
      // Add other fields from your form if they match the backend model
      // e.g., anonymous: feedbackForm.anonymous
    };
    ...
    try {
      ...
      setFormSuccess("Campaign created and activated successfully!");

      // Optionally, switch back to the feedback management tab after success
      setTimeout(() => {
  ...
  const handleDeleteCampaign = (campaignId) => {
    if (
      window.confirm(
        "Are you sure you want to delete this feedback campaign? This action cannot be undone."
      )
    ) {
  ...
  const renderFeedbackManagement = () => (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Manage all your feedback campaigns
          </p>
        </div>
        <button
          onClick={() => {
      ...
      {/* Quick Stats - Now calculated from REAL data */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {/* Note: The 'active' status in your mock data might be 'open' in the backend. Adjust if needed. */}
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">
                {
                  activeFeedbackCampaigns.filter((c) => c.status === "open")
                    .length
                }
              </p>
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-200/50">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">
                {
                  activeFeedbackCampaigns.filter((c) => c.status === "draft")
                    .length
                }
              </p>
        ...
        {/* These stats are commented out as the data doesn't exist in the backend model yet */}
        {/* We can add these fields later! */}
        <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-200/50 opacity-50">
          <div className="flex items-center space-x-3">
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-200/50">
        ...
        {campaignsLoading ? (
          ...
          <div className="p-12 text-center text-red-600">{campaignsError}</div>
        ) : activeFeedbackCampaigns.length === 0 ? (
          <div className="p-12 text-center text-gray-500">
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    {/* Campaign Details - Using REAL data fields */}
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600">
                      <div className="flex items-center space-x-2">
                        <GraduationCap className="w-4 h-4" />
                        <span>{campaign.targetDepartment}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Users className="w-4 h-4" />
                        <span>Year {campaign.targetYear}</span>
                      </div>
                    </div>
                  </div>
                  {/* Action Buttons */}
                  <div className="flex items-center space-x-2 ml-4">
                    {/* These buttons can be wired up next */}
                  </div>
                </div>

                {/* Campaign Stats - Using REAL data */}
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div className="bg-green-50 rounded-lg p-4">
                    ...
                    {/* Calculate question count from the array length */}
  ...
  const renderCreateFeedback = () => (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Design a comprehensive feedback form for students
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4">
        {formError && (
          <div className="bg-red-100 text-red-700 p-3 rounded-lg text-center animate-pulse">
            {formError}
          </div>
        )}
        {formSuccess && (
          <div className="bg-green-100 text-green-700 p-3 rounded-lg text-center animate-pulse">
            {formSuccess}
          </div>
        )}
      </div>
              ...
              <label className="flex items-center space-x-3 p-4 border-2 border-gray-200 rounded-xl hover:border-blue-300 cursor-pointer transition-all">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-500">
                    Students' identities will not be recorded
                  </div>
                </div>
              </label>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-100">
              Here's what's happening at {adminInfo.institute} today
            </p>
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {campaign.department} • {campaign.faculty}
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <div className="text-xs text-gray-500">
                    {Math.round(
                      (campaign.responses / campaign.totalStudents) * 100
                    )}
                    % complete
                  </div>
                </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="font-semibold text-gray-900">
                {
                  activeFeedbackCampaigns.filter((c) => c.status === "active")
                    .length
                }
              </span>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-semibold text-gray-900">
                {activeFeedbackCampaigns.reduce(
                  (acc, c) => acc + c.responses,
                  0
                )}
              </span>
            </div>
  ...
  const renderAnalytics = () => (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-600 mt-1">
          Comprehensive insights into feedback performance
        </p>
      </div>
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "campaigns":
        return <CreateCampaignForm />; // <-- This is the main change
      case "dashboard":
      ...
      case "courses":
        return renderPlaceholderSection(
          "Course Management",
          "Organize and manage courses and subjects"
        );
      case "settings":
        return renderPlaceholderSection(
          "Settings",
    ...
    <div className="min-h-screen bg-gray-50">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className="ml-2 font-medium">
                      {new Date(selectedFeedback.deadline).toLocaleDateString()}
                    </span>
                  </div>
                </div>
                ...
                <div className="pt-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-gray-600">
                      ...
                      {selectedFeedback.totalStudents}
                    </span>
                  </div>
      ...
      <div className="p-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <p>
            This is your protected admin dashboard. You can now start building
            the features to create and manage feedback forms.
          </p>
        </div>
      </div>
    </div>
