...
const StudentPortal = () => {
  ...
  const achievements = [
    { id: 1, title: 'Feedback Champion', description: 'Completed 10+ feedback forms', icon: '🏆', color: 'yellow', earned: true },
    { id: 2, title: 'Early Bird', description: 'First to submit feedback 5 times', icon: '🌅', color: 'blue', earned: true },
    { id: 3, title: 'Constructive Critic', description: 'Detailed feedback appreciated by faculty', icon: '💡', color: 'purple', earned: true },
    { id: 4, title: 'Consistency King', description: 'Submit feedback for 30 days straight', icon: '👑', color: 'orange', earned: false },
    { id: 5, title: 'Community Helper', description: 'Help 100+ students with feedback', icon: '🤝', color: 'green', earned: false },
    { id: 6, title: 'Master Reviewer', description: 'Complete 50+ comprehensive reviews', icon: '⭐', color: 'pink', earned: false }
  ];
  ...
  const sidebarItems = [
    ...
    { id: 'analytics', label: 'My Analytics', icon: <BarChart3 className="w-5 h-5" /> },
    ...
    return (
      <div className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-200/50 group">
        <div className="flex items-start justify-between mb-4">
          <div className={`w-16 h-16 bg-gradient-to-r ${colorClasses[course.color]} rounded-2xl flex items-center justify-center group-hover:scale-105 transition-transform duration-300`}>
            <BookOpen className="w-8 h-8 text-white" />
          </div>
          <span className={`px-3 py-1 text-xs font-semibold rounded-full ${statusColors[course.status]}`}>
            {course.status}
          </span>
        </div>

        <h3 className="text-xl font-bold text-gray-900 mb-2">{course.name}</h3>
        <p className="text-gray-600 mb-1">{course.code} • {course.faculty}</p>
        ...
        <div className="mb-4">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div 
              className={`bg-gradient-to-r ${colorClasses[course.color]} h-2 rounded-full transition-all duration-500`}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-1">
            {[...Array(5)].map((_, i) => (
              <Star 
                key={i} 
                className={`w-4 h-4 ${i < course.rating ? 'text-yellow-400 fill-current' : 'text-gray-300'} cursor-pointer hover:text-yellow-400 transition-colors`}
                onClick={() => {
            ...
            <span className="text-sm text-gray-500 ml-2">{course.rating || 'Rate'}</span>
          </div>
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-purple-600 via-pink-600 to-blue-600 rounded-3xl p-8 text-white overflow-hidden relative">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold mb-2">Hey {studentData.name}! 👋</h1>
              <p className="text-purple-100 mb-4">Ready to share your thoughts and shape your education?</p>
              <div className="flex items-center space-x-6">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold">{studentData.completedFeedbacks}</div>
                  <div className="text-sm text-purple-100">Feedback Given</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold">{studentData.achievementPoints}</div>
            ...
            <div className="hidden md:flex items-center space-x-4">
              <div className="text-right">
                ...
                <p className="text-purple-100 text-sm">{studentData.semester}</p>
              </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard 
          ...
          color="yellow"
          subtitle={`${achievements.length - achievements.filter(a => a.earned).length} more to unlock`}
        />
        <StatCard 
          title="Feedback Score" 
          value={`${Math.round((studentData.completedFeedbacks / (studentData.completedFeedbacks + studentData.pendingFeedbacks)) * 100)}%`}
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-200/50">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-bold text-gray-900">Pending Feedback</h3>
          ...
          <div className="space-y-4">
            {enrolledCourses.filter(course => course.status === 'pending' || !course.lastFeedback).slice(0, 2).map((course) => (
              <div key={course.id} className="flex items-center justify-between p-4 bg-gradient-to-r from-orange-50 to-yellow-50 rounded-xl border border-orange-200/50">
                <div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-200/50">
          ...
          <div className="space-y-4">
            {achievements.filter(a => a.earned).slice(0, 3).map((achievement) => (
              <div key={achievement.id} className="flex items-center space-x-4 p-4 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl border border-yellow-200/50">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-200/50">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-gray-900">Recent Feedback Activity</h3>
                ...
                <div className="flex items-center justify-between text-xs text-gray-500">
                  ...
                  <div className="flex items-center space-x-3">
                    <button
                      ...
                      onClick={() => {
                        setFeedbackList(list => list.map(f => f.id === feedback.id ? {...f, likes: f.likes + 1} : f));
                      }}
                    >
                      <ThumbsUp className="w-4 h-4" />
                      <span>{feedback.likes}</span>
                    </button>
                    <button className="hover:text-blue-600"><Share className="w-4 h-4" /></button>
                  </div>
                </div>
              </div>
            </div>
          ...
          {filtered.length === 0 && (
            <div className="col-span-full">
              <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center">
                ...
                <p className="text-gray-700 font-medium">No courses matched your search.</p>
              </div>
            </div>
          )}
        </div>
  ...
  const renderAchievements = () => (
    <div className="space-y-8">
      <div className="text-center">
        ...
        <p className="text-gray-600 mb-8">Keep giving great feedback to unlock more badges!</p>
        
        <div className="flex items-center justify-center space-x-8 mb-12">
          <div className="text-center">
            <div className="text-4xl font-bold text-purple-600">{achievements.filter(a => a.earned).length}</div>
            <div className="text-gray-600">Earned</div>
          </div>
          <div className="text-center">
            <div className="text-4xl font-bold text-orange-600">{achievements.length - achievements.filter(a => a.earned).length}</div>
            <div className="text-gray-600">Available</div>
          </div>
          <div className="text-center">
            <div className="text-4xl font-bold text-green-600">{studentData.achievementPoints}</div>
        ...
        {achievements.map((achievement) => (
          ...
          >
            <div className="text-center">
              ...
              <h3 className={`text-xl font-bold mb-2 ${achievement.earned ? 'text-gray-900' : 'text-gray-500'}`}>
                {achievement.title}
              </h3>
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          onClick={() => {
            const firstPending = enrolledCourses.find(c => c.status === 'pending' || !c.lastFeedback) || enrolledCourses[0];
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {feedbackList.map((fb) => (
          <div key={fb.id} className="bg-white rounded-2xl p-6 shadow-lg border border-gray-200/60">
            <div className="flex items-start justify-between mb-3">
              <div>
                <p className="font-semibold text-gray-900">{fb.course}</p>
                <p className="text-sm text-gray-600">{fb.faculty}</p>
              </div>
              <span className="text-xs px-2 py-1 rounded-full bg-gray-100 text-gray-700">{fb.date}</span>
            </div>
            ...
            <div className="flex items-center justify-between text-sm">
              <span className={`px-2 py-1 rounded-full ${
                fb.status === 'submitted' ? 'bg-blue-100 text-blue-800' :
                fb.status === 'reviewed' ? 'bg-green-100 text-green-800' : 'bg-purple-100 text-purple-800'
              }`}>{fb.status}</span>
              <div className="flex items-center space-x-3 text-gray-600">
                <button
                  className="flex items-center space-x-1 hover:text-blue-600"
                  onClick={() => setFeedbackList(list => list.map(f => f.id === fb.id ? {...f, likes: f.likes + 1} : f))}
                >
                  <ThumbsUp className="w-4 h-4" />
                  <span>{fb.likes}</span>
                </button>
                <button className="hover:text-purple-600"><Edit className="w-4 h-4" /></button>
                <button
                  className="hover:text-red-600"
                  onClick={() => setFeedbackList(list => list.filter(f => f.id !== fb.id))}
                >
  ...
  const renderAnalytics = () => {
    const avgCourseProgress = Math.round(enrolledCourses.reduce((s,c)=>s+c.progress,0)/enrolledCourses.length);
    const avgRating = Math.round(
      (enrolledCourses.reduce((s,c)=>s+(c.rating||0),0)/(enrolledCourses.filter(c=>c.rating>0).length||1))*10
    )/10;
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard title="Avg Course Progress" value={`${avgCourseProgress}%`} icon={<Activity className="w-6 h-6 text-white" />} color="purple" />
          <StatCard title="Avg Rating Given" value={`${avgRating}/5`} icon={<Star className="w-6 h-6 text-white" />} color="yellow" />
          <StatCard title="Total Feedback" value={feedbackList.length} icon={<MessageSquare className="w-6 h-6 text-white" />} color="blue" />
          <StatCard title="Pending Items" value={studentData.pendingFeedbacks} icon={<Clock className="w-6 h-6 text-white" />} color="orange" />
        </div>

        <div className="bg-white rounded-2xl border border-gray-200 p-6">
          <h3 className="font-semibold text-gray-900 mb-4">Course Progress Overview</h3>
          <div className="space-y-4">
              ...
              <div key={c.id}>
                ...
                <div className="w-full h-2 bg-gray-200 rounded-full">
                  <div className={`h-2 rounded-full bg-gradient-to-r ${
                    c.color === 'blue' ? 'from-blue-500 to-blue-600' :
                    c.color === 'green' ? 'from-green-500 to-green-600' :
                    c.color === 'purple' ? 'from-purple-500 to-purple-600' :
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6">
          ...
          <div className="flex flex-wrap gap-3">
            <button className="px-4 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-800 flex items-center space-x-2">
              <Download className="w-4 h-4" /><span>Feedback Summary (PDF)</span>
            </button>
            <button className="px-4 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-800 flex items-center space-x-2">
              <Download className="w-4 h-4" /><span>Course Progress (CSV)</span>
            </button>
          </div>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            <label className="flex items-center justify-between p-3 border rounded-xl">
              ...
              <input type="checkbox" checked={prefEmail} onChange={() => setPrefEmail(v=>!v)} className="w-5 h-5" />
            </label>
            <label className="flex items-center justify-between p-3 border rounded-xl">
              <span className="text-gray-700">Push notifications</span>
              <input type="checkbox" checked={prefPush} onChange={() => setPrefPush(v=>!v)} className="w-5 h-5" />
            </label>
            <label className="flex items-center justify-between p-3 border rounded-xl">
              <span className="text-gray-700">Product tips</span>
              <input type="checkbox" checked={prefTips} onChange={() => setPrefTips(v=>!v)} className="w-5 h-5" />
            </label>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            {['light','system','dark'].map(opt => (
              <button
                ...
                className={`px-4 py-2 rounded-lg border ${theme===opt ? 'bg-purple-600 text-white border-purple-600' : 'bg-white text-gray-800 border-gray-300'} capitalize`}
              >
                {opt}
              </button>
            ))}
          </div>
          <p className="text-sm text-gray-500 mt-2">Theme selection is visual only in this demo.</p>
        </div>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-3xl p-8 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
          ...
          <div className="mb-6 p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl border border-purple-200">
            <h4 className="font-bold text-gray-900 mb-1">{selectedCourse.name}</h4>
            <p className="text-gray-600 text-sm">{selectedCourse.code} • {selectedCourse.faculty}</p>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => {
                console.log('Feedback submitted:', { course: selectedCourse.id, rating, feedback: feedbackText });
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50">
              ...
              {sidebarItems.map(item => (
                <button
                  ...
                  className={`w-full flex items-center space-x-3 px-3 py-2 rounded-xl ${
                    activeTab === item.id ? 'bg-purple-600 text-white' : 'text-gray-700 hover:bg-gray-100'
                  }`}
                >
                  {item.icon}
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-6">
        ...
        <header className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            <button className="lg:hidden p-2 rounded-lg hover:bg-white" onClick={() => setSidebarOpen(true)}>
            ...
            <div className="hidden lg:flex items-center space-x-3">
              ...
              <div>
                <p className="font-bold text-gray-900">Student Portal</p>
                <p className="text-xs text-gray-500">{studentData.semester} • {studentData.branch}</p>
              </div>
            </div>
          </div>
